...
- **Buyer mode** — the current build is a 5-tab read-only portal (Home, Chatbot, Contract History, ROI Calculator, Vendor Contacts) presenting one renewal. The Home tab renders a stacked dashboard: a renewal banner (company, days-to-renewal, annual spend, status badge, AE/CSM names), four contract metric cards (Annual Spend, Seats Purchased, Seats Active, Cost/Seat), an Action Items checklist (with owner and due date), and a Key Dates timeline. The Chatbot and Vendor Contacts have been split into dedicated full-width tabs. The originally intended 3-column Home layout — contract context left / proposed order form center / renewal assistant + contacts right — and the Renewals Deck tab are planned but not yet implemented.
- **Admin mode** — a master-detail seller dashboard *(planned, not yet implemented — no `src/components/Admin/` exists)*: a pipeline list of accounts with inline engagement signals (left), and a per-account detail pane (right) leading with **stakeholder visibility** and engagement telemetry.
...
**Data boundary:** all content is fake data in `src/data/`. Going to production = replacing those exports with API calls; the UI is unchanged. Current data files: `renewalData.js`, `contractHistory.js`, `roiData.js`, `vendorContacts.js`. The planned integration contract for the Qualification agent (`src/data/qualificationOutput.js`) does not yet exist in the codebase.

## Detailed Design — Feature Set

### Buyer portal

**Home tab (current build — stacked dashboard):**
- **Renewal banner** — vendor name, product, days-to-renewal (color-coded: ≤60d red / ≤120d amber), annual spend, status badge (On Track / Review Needed / At Risk), AE and CSM names. Driven by `src/data/renewalData.js`.
- **Contract metric cards** — four at-a-glance cards: Annual Spend (with YoY delta), Seats Purchased, Seats Active (utilization %), Cost/Seat.
- **Action Items** — task checklist with per-item owner label and due date; progress bar showing completed vs. total.
- **Key Dates** — chronological timeline with critical / warning / info severity classifications.

*Planned, not yet implemented (original 3-column design):* Highlights toggle, Proposed Order Form with Redlines toggle and Pricing Justification badge, renewal-option selector, "Your products" Included/Add-on list, Renewal assistant panel (currently its own Chatbot tab), and Contacts panel (currently its own Vendor Contacts tab). Deck teaser also planned once Renewals Deck tab ships.

**Contract History tab** — a table of prior contracts (Term, Annual Value, Seats, Status, Signed date, Notes) drawn from `src/data/contractHistory.js`. The fuller grouped document library — Legal & Compliance, Product Briefs, Implementation & Support groups; format badges (PDF/DOCX/Link); Updated / New / Required-to-sign tags — is planned but not yet implemented.

**ROI Calculator tab** — three input sliders (typeable) — hours saved per user per month, average fully-loaded hourly rate, active users — compute ROI and payback against annual cost. Includes a Benefit Breakdown table of additional line-item benefits from `src/data/roiData.js`. This is the Finance-proof artifact buyers otherwise build themselves.

**Chatbot tab** — a full-width conversational interface ("Renewals Bot") with canned keyword-matched replies keyed to the current renewal's data. Suggested prompts shown as chips. Previously designed as a panel in the Home tab's right column; implemented as a standalone tab in the current build.

**Vendor Contacts tab** — a card grid of seller-side contacts (name, role, team badge, email, phone) drawn from `src/data/vendorContacts.js`. Previously designed as a contacts panel in the Home tab's right column. Schedule-a-call modal and Buyer Renewal Team / Seller Account Team separation are planned.

**Renewals Deck tab** *(planned, not yet built)* — the Proposal agent's tailored deck (cover → pricing → next steps), expandable slide-by-slide with keyboard nav. Shown only when a proposal has been prepared.

**Quick Renew** *(planned, not yet in current build)* — a header CTA with a confirmation modal for frictionless flat renewal.

**Planned, not yet built:** a "What's new since you bought this" module — features shipped *during the contract term* that the buyer actively uses, as internal justification ammunition (Courtney's headline ask). The ROI Calculator is the closest currently built surface; it does not yet tie shipped features to the buyer's usage. This is the highest-value next addition to the buyer portal.

### Admin dashboard (seller)

> **Implementation status:** not yet built. No `src/components/Admin/` directory exists in the current codebase. All content below is the planned design.
...
> **Implementation status:** `src/data/qualificationOutput.js` does not yet exist in the codebase. The schema below is the planned contract.
