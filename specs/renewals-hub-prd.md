...
- **Buyer mode** — a buyer portal presenting one renewal, currently shipping five tabs: **Home** (renewal banner, metric cards, action items, key dates), **Chatbot** (renewal assistant implemented as a standalone tab rather than the planned Home right column), **Contract History**, **ROI Calculator**, and **Vendor Contacts** (standalone tab rather than the planned Home right column). The Renewals Deck tab and the full 3-column Home layout are designed but not yet built.
- **Admin mode** — a master-detail seller dashboard *(designed but not yet built — no admin components exist in `src/`)*: a pipeline list of accounts with inline engagement signals (left), and a per-account detail pane (right) leading with **stakeholder visibility** and engagement telemetry.
...
**Data boundary:** all content is fake data in `src/data/`. Going to production = replacing those exports with API calls; the UI is unchanged. The intended integration contract for the Qualification agent is `src/data/qualificationOutput.js` (not yet created; current placeholder data lives in `src/data/renewalData.js`, `contractHistory.js`, `roiData.js`, and `vendorContacts.js`).

## Detailed Design — Feature Set

### Buyer portal

**Home tab** *(currently built: renewal banner, four metric cards, action items checklist, key dates timeline — `src/components/Home/HomeTab.jsx`; the 3-column design below is the target)*:
...
- **Renewal assistant (right)** — context-aware chatbot keyed to the renewal (canned replies in demo). *(Implemented as a standalone **Chatbot** tab, `src/components/Chatbot/ChatbotTab.jsx`, rather than the planned right column.)*
- **Contacts (right)** — Buyer Renewal Team and Seller Account Team, with a Calendly-style **Schedule-a-call** modal. *(Implemented as a standalone **Vendor Contacts** tab, `src/components/VendorContacts/VendorContactsTab.jsx`, showing seller contacts by team with email/phone; the Buyer team grouping and Schedule-a-call modal are not yet built.)*
- **Deck teaser (right, conditional)** — compact preview of the Renewals Deck when a proposal exists.

**Contract History tab** (`src/components/ContractHistory/ContractHistoryTab.jsx`) — a table of prior contracts with columns for term, annual value, seats, status, signed date, and notes (data from `src/data/contractHistory.js`). The broader grouped document library (Legal & Compliance, Product Briefs, Implementation & Support groups; format badges PDF/DOCX/Link; Updated/New/Required tags) is planned but not yet built.
...
**Renewals Deck tab** *(not yet built — not in the current tab list)* — the Proposal agent's tailored deck (cover → pricing → next steps), expandable slide-by-slide with keyboard nav. Shown only when a proposal has been prepared.

**Quick Renew** — header CTA with a confirmation modal for frictionless flat renewal (visual-only in demo).

**Planned, not yet built:** the full 3-column Home layout (proposed order form, Highlights toggle, Redlines toggle, Pricing Justification badge, renewal-option selector, Deck teaser), the Renewals Deck tab, the Quick Renew header CTA, and the Admin dashboard (see below). Additionally: a "What's new since you bought this" module — features shipped *during the contract term* that the buyer actively uses, as internal justification ammunition (Courtney's headline ask). The closest currently built surface is the ROI Calculator; it does not yet tie shipped features to the buyer's usage. This is the highest-value next addition to the buyer portal.

### Admin dashboard (seller)

*(Not yet built — no admin components exist in `src/components/` as of this writing; the design below is the target spec.)* The dashboard is **strict master-detail**, per Evan's 1:1 (Jun 19): *"first I'm seeing a summary of accounts and once I click into an account, I'm only seeing the account."* The summary and the account never coexist on screen — the original failure of the bucketed Portfolio Snapshot was that it stayed pinned above the account detail and read as a whole-product dashboard.
...
`qualificationOutput.js` per account *(not yet created in `src/data/`)*:
```
